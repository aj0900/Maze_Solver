...
void writeToFile(char filename[], int row, int col, int old, int new){
  FILE *pointer;
...
void maze_solve_recursive(maze_data_t *maze_data, int current_row, int current_col, int current_distance) {
    ...
    if (this_cell_distance == EMPTY || current_distance < this_cell_distance)
       mazedata_set(maze_data, current_row, current_col, current_distance);
        ...
        writeToFile("journal", current_row, current_col, 0, current_distance);
...
void journal_solver(char filename[], maze_data_t maze)
  {
    int row, col, new, old;

    FILE *pointerx;

    pointerx = fopen(filename, "r");
    while (!feof (pointerx))
   {
     fscanf(pointerx, "%d %d %d %d", &row, &col, &old, &new);
     maze.maze[row][col] = new;
   }
    fclose(pointerx);
  }
...
For some reason opt is not being picked up in the while loop
If oflag */
    ...
    for (int arg=1; arg < argc; arg++) {
        ...
        journal_solver("journal.txt", maze_data);
